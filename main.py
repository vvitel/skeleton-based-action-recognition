...
def get_parser():
    ...
    parser.add_argument('--save-score',
                        type=str2bool,
                        default=False,
    ...
    parser.add_argument('--model_name',
                        default='stgcn',
                        help='the name of the model will be used')
    ...
    parser.add_argument('--ignore-weights',
                        type=str,
                        default=[],
                        nargs='+',
    ...
    parser.add_argument('--topology',
                        ...
                        default=[],
                        help='model topology')
    parser.add_argument('--layer_threshold',
                        default=1e-4,
                        help='the threshold to stop adding layers')
    parser.add_argument('--block_threshold',
                        default=1e-4,
...
class Processor():
        ...
        if save_model:
            ...
            if arg.model_name.lower() in ["pstgcn", "pstbln"]:
                torch.save(weights,
                           ...
                           str(self.arg.model_args['topology'][-1]) + '.pt')
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                with torch.no_grad():
                    ...
                    #_, predict_label = torch.max(output.data, 1) #code de base
                    predict_label = torch.softmax(output.data, dim=0)[:,1] #modifié
                    step += 1
    ...
    def prog_init(self, block_iter):
        ...
        for current_key in self.model.state_dict():
            ...
            if ('g_conv' or 'gcn_residual' or 'bln_residual' or 'tcn.t_conv.bias' or 'residual' or 'bn.weight'
                or 'bn.bias' or 'bn.running_mean' or 'bn.running_var') in current_key:
                if current_key in old_keys:
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            elif arg.model_name.lower() in ["pstgcn", "pstbln"] and self.arg.model_args['topology'] == []:
                acc_layer_old = acc_block_old = acc_layer_new = acc_block_new = 1e-10
                loss_layer_old = loss_block_old = loss_layer_new = loss_block_new = 1e+10
